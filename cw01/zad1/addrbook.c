...
void addr_book_free(AddrBook *book) {
    for (AddrBookInterator iter = addr_book_iter(book); addr_book_has_next(&iter); iter = addr_book_next(iter)) {
...
AddrBookInterator addr_book_next(AddrBookInterator iter) {
    ...
    if (result._book->_backend == ABB_LinkedList) {
        result._cur_ll = ll_next(result._cur_ll);
    } else if (result._book->_backend == ABB_Bst) {
        result._cur_bst = bst_next(result._cur_bst);
    }
    return result;
}
